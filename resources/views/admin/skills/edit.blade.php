<x-layout-admin>
    <x-slot:title>Modifier une compétence — Admin</x-slot:title>

    <div class="site-container max-w-4xl flex flex-col gap-8">
        <div>
            <span class="font-mono text-hk-green text-sm">// admin</span>
            <h1 class="font-mono text-3xl font-bold text-hk-text mt-2">Modifier la compétence</h1>
        </div>
        ...
        <form action="{{ route('admin.skills.update', $skill) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                          ...
                          <input type="text" name="name" value="{{ old('name', $skill->name) }}" class="form-input" />
                    @error('name')<p class="text-red-400 text-xs mt-2 font-mono">{{ $message }}</p>@enderror
                </div>

                <div>
                          <label class="form-label">Groupe</label>
                          <input type="text" name="group" value="{{ old('group', $skill->group) }}" list="skill-groups" placeholder="Backend, Frontend, AI/MLOps, DevOps" class="form-input" />
                ...
                <div>
                          ...
                          <input type="number" name="order" value="{{ old('order', $skill->order) }}" class="form-input" />
                    @error('order')<p class="text-red-400 text-xs mt-2 font-mono">{{ $message }}</p>@enderror
                </div>

                <div>
                          <label class="form-label">Icone (optionnel)</label>
                          <input type="text" name="icon" value="{{ old('icon', $skill->icon) }}" class="form-input" />
            ...
            <div class="flex items-center gap-4">
                ...
                <button type="submit" class="btn-primary font-mono">Mettre à jour</button>
            </div>
        </form>
    </div>
</x-layout-admin>
